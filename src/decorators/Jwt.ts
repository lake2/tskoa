...
export function Jwt(target: any, key: string, index: number): void;
export function Jwt<T>(dto?: Class<T>): any;
...
function decorate(target: any, key: string, index: number, dto?: Class<any>) {
    ...
    if (parameters.some(t => t.decorator === "jwt")) {
        throw new Error(`'${target.constructor.name}.${key}()' has multiple Jwt decorator.`);
    }

    parameters.push({ index, decorator: "jwt", dto });
